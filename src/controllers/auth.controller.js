...
/**
 * @description 사용자 회원가입을 처리하는 컨트롤러 (DB 버전 - 임시 비활성화)
 */
export const register = async (req, res, next) => {
  // 🔥 임시로 DB 없는 버전으로 리다이렉트
  return registerTest(req, res, next);
};

/**
 * @description 사용자 로그인을 처리하는 컨트롤러 (DB 버전 - 임시 비활성화)
 */
export const login = async (req, res, next) => {
  // 🔥 임시로 DB 없는 버전으로 리다이렉트
  return loginTest(req, res, next);
};
...
/**
 * @description 토큰 유효성을 검증하는 컨트롤러 (DB 버전 - 임시 비활성화)
 */
export const verify = async (req, res, next) => {
  // 🔥 임시로 DB 없는 버전으로 리다이렉트
  return verifyTest(req, res, next);
};
